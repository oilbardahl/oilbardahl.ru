...
<? if ($arResult['PROPERTIES']['vote_sum']['VALUE'] && $arResult['PROPERTIES']['vote_count']['VALUE']):?>
	<?$ratingVisible = floor($arResult['PROPERTIES']['vote_sum']['VALUE']/$arResult['PROPERTIES']['vote_count']['VALUE'])?>
<? endif?>
<ul class="list-inline product-ratings margin-bottom-15 runs-vote" style="display:table;" id="<?=$arJSParams["starsId"]?>">
	<? if ($ratingVisible):?>
		<? for($i=0;$i<5;$i++):?>
...
BX.ready(function(){
	...
	window.<?=$strObName?>.setValue("<?=$votesCount > 0 ? $ratingVisible : 0?>");
});
</script>