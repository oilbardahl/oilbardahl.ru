...
// ‘айл принимает параметры, переданные методом GET и только в режиме PAYMENT
if($mode == "PAYMENT")
{
	if(IntVal($issuer_id)>0)
	{
		...
		if($bCorrectPayment)
		{
			...
			$strPS_STATUS_DESCRIPTION .= "код подтверждени€ платежа - ".$confirmed."";

			$strPS_STATUS_MESSAGE = "";
			if (isset($payer) && strlen($payer)>0)
				$strPS_STATUS_MESSAGE .= "e-mail покупател€ - ".$payer."; ";
		...
		echo " од заказа не задан";
}
else
	echo "¬ид операции не PAYMENT";
?>
