...
Class CImaginwebSms 
{
	...
	public static function GetOrderPropsValue($ORDER_ID)
	{
		$ORDER_ID = intval($ORDER_ID);
		$arProps = array();
		$arFilter = array('ORDER_ID' => $ORDER_ID);
		$dbProps = CSaleOrderPropsValue::GetList(array(), $arFilter, false, false, array('*'));
		while($arProp = $dbProps->Fetch())
		{
			$arProp['~VALUE'] = $arProp['VALUE'];
			
			if(!empty($arProp['VALUE_ORIG']))
			{
				$arProp['VALUE'] = $arProp['VALUE_ORIG'];
				
				if(in_array($arProp['PROP_TYPE'], array('SELECT','MULTISELECT','RADIO')))
				{
					$val = '';
					if(is_array($arProp['VALUE']))
					{
						foreach($arProp['VALUE'] as $v)
						{
							$arVal = CSaleOrderPropsVariant::GetByValue($arProp['ORDER_PROPS_ID'], $v);
							if($arVal)
								$val[] = $arVal['NAME'];
						}
					}
	...
	function OnSaleComponentOrderOneStepCompleteHandler($id,$arFields)
	{
		...
		if(strlen(trim($code))>0 && $switch != 'OFF') {
			...
			$arProps = self::GetOrderPropsValue($id);
			$phone = '';
			foreach($arProps as $arProp)
			{
				if($arProp['CODE'] == $code) $phone = $arProp['VALUE'];
				if(in_array($arProp['CODE'],$arReplaces)) $arReplaces['#PROP_'.$arProp['CODE'].'#'] = implode(', ', (array)$arProp['VALUE']);
			}
