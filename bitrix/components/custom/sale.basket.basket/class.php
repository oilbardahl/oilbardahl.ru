...
class CBitrixBasketComponent extends CBitrixComponent
{
	...
	public function recalculateBasket($arPost)
	{
		...
		while ($arItem = $dbItems->Fetch())
		{
			if (CSaleBasketHelper::isSetItem($arItem))
				continue;
			$arInCartProduct[$arItem['ID']] = $arItem['PRODUCT_ID'];
		}


		foreach($arPost as $keyPostItem=>$arPostItem):
			if (strpos($keyPostItem,'QUANTITY_')!==false):
				$explodeItemAr = explode('QUANTITY_',$keyPostItem);
				if (!in_array($explodeItemAr[1],$arInCartProduct) && $arPostItem):
					...
					elseif(in_array($explodeItemAr[1],$arInCartProduct) && empty($arPostItem)):
					   CSaleBasket::Delete(array_search($explodeItemAr[1], $arInCartProduct));
				endif;
			endif;
		endforeach;
		...
		foreach ($arTmpItems as $arItem)
		{
			...
			if (!isset($arPost["QUANTITY_".$arItem["PRODUCT_ID"]]) || floatval($arPost["QUANTITY_".$arItem["PRODUCT_ID"]]) <= 0)
			{
				$quantityTmp = ($isFloatQuantity === true) ? floatval($arItem["QUANTITY"]) : intval($arItem["QUANTITY"]);
			}
			else
			{
				//AddMessage2Log("QUANTITY_".$arItem["PRODUCT_ID"]);
				//AddMessage2Log($arPost["QUANTITY_".$arItem["PRODUCT_ID"]]);
				$quantityTmp = ($isFloatQuantity === true) ? floatval($arPost["QUANTITY_".$arItem["PRODUCT_ID"]]) : intval($arPost["QUANTITY_".$arItem["PRODUCT_ID"]]);
			}
