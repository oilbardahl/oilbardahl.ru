	...
	*/
	...
	public function GetCreditBalance($arParams = array(), $encoding = LANG_CHARSET) {
		...
		if($gate == 'smsdirect.ru') {
			...
			//#$query = "http://gate.sms-manager.ru/_balance.php?user=".$auth['login']."&password=".$auth['password'];
			//debmes(htmlspecialchars($result),$query);
			//#$result = file_get_contents($query);
...
';
	...
	public function Send($phone, $message, $arParams = array(), $encoding = LANG_CHARSET)
	{
		...
		#debmes($allOriginator,'sender'.$SITE_ID);
		...
		if(CIWebSMS::CheckPhoneNumber($phone) && $gate == 'qtelecom.ru') {
			...
			$user = COption::GetOptionString('imaginweb.sms', 'username_qtelecom.ru'); // Ваш логин при регистрации, также его называют System_ID
			$pass = COption::GetOptionString('imaginweb.sms', 'password_qtelecom.ru');   // Ваш пароль к логину
		...
		if(CIWebSMS::CheckPhoneNumber($phone) && $gate == 'redsms.ru2.0') {
			...
			$user = COption::GetOptionString('imaginweb.sms', 'username_redsms.ru2.0'); // Ваш логин при регистрации, также его называют System_ID
			$pass = COption::GetOptionString('imaginweb.sms', 'password_redsms.ru2.0');   // Ваш пароль к логину
		...
		if(CIWebSMS::CheckPhoneNumber($phone) && $gate == 'smsdirect.ru') {
			...
			$auth['login'] = urlencode($auth['login']);
			//$query = "https://www.smsdirect.ru/get_user_info?login=".$auth['login']."&pass=".$auth['password']."&mode=0";
			$query = "https://www.smsdirect.ru/submit_message?login=".$auth['login']."&pass=".$auth['password']."&from=".$originator."&to=".$phone."&text=".$message;
			$ch = curl_init();
			curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, FALSE);
			curl_setopt($ch, CURLOPT_HEADER, false);
			curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);
