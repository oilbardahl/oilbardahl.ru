;(function(window) {
	...
	JCIblockVoteStars.prototype.onMouseMove = function(event)
	{
		...
		var voteValue = (event.pageX - starsPos.left)/($('.runs-vote li:first-child').width()+8);
		...
		$('.runs-vote li i').removeClass('rating-selected');
		$('.runs-vote li i').addClass('rating');
		$('.runs-vote li').each(function(j){			if (j<voteValue){
				$(this).find('i').addClass('rating-selected').removeClass('rating');
			}
		});
	...
	{
	    $('.runs-vote li i').removeClass('rating-selected').addClass('rating');
	...
	JCIblockVoteStars.prototype.onVote = function(event)
	{
		...
		this.unBindEvents();
		this.ajaxParams.rating = ($('.runs-vote li i.rating-selected').length-1);
