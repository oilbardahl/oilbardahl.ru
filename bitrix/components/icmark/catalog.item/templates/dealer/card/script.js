...
function skuPropClickHandler(e)
{
	...
	if (!!target && target.hasAttribute('data-value-id'))
	{
		...
		BX.ajax({
			url: '/bitrix/components/custom/sale.basket.basket/ajax.php',
			method: 'POST',
			data: postData,
			dataType: 'json',
...
function enterCoupon()
{
	...
	if (!!items && $(items).find("input[name^=QUANTITY_]").length > 0)
	{
		$(items).find("input[name^=QUANTITY_]").each(function(){
	...
	if (ajax_request_recalc){
		ajax_request_recalc.abort();
	}

	var ajax_request_recalc = $.ajax({
	    type: "POST",
	    url: '/bitrix/components/custom/sale.basket.basket/ajax.php',
	    data: postData,
	    dataType: 'jsonp',
...
function recalcBasketAjax(deleteRowParam)
{
	BX.showWait();
