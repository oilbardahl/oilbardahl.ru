...
if($POST_RIGHT >= "R"):
	if(CModule::IncludeModule("sale")) {
		$db_props = CSaleOrderProps::GetList(array("SORT" => "ASC"),array("TYPE" => array('TEXT','TEXTAREA','STRING','NUMBER','ENUM','DATE')));
		$arSaleProps = array();
		$arReplaces = array();
		while($arProps = $db_props->Fetch())
		{
			if((!isset($arSaleProps[$arProps['CODE']]) || !in_array($arProps['NAME'],$arSaleProps[$arProps['CODE']])) && strlen($arProps['CODE'])>0 && in_array($arProps['TYPE'], array('TEXT','TEXTAREA')))
				$arSaleProps[$arProps['CODE']][] = $arProps['NAME'];
			if((!isset($arReplaces['PROP_'.$arProps['CODE']]) || !in_array($arProps['NAME'],$arReplaces['PROP_'.$arProps['CODE']])) && strlen($arProps['CODE'])>0)
				$arReplaces['PROP_'.$arProps['CODE']][] = $arProps['NAME'];
		}
		...
		$arOrderFields = array();
		$db_sales = CSaleOrder::GetList(array("ID" => "DESC"), array(), false, array('nTopCount' => 1));
		$ar_sales = $db_sales->Fetch();
		foreach($ar_sales as $k => $v)
		{
			$arOrderFields[] = $k;
		}
	}
	...
	$gateDef = trim(COption::GetOptionString('imaginweb.sms', 'gate'));
	if($gateDef != '') {
		
		foreach($arSites as $siteId => $arVal) {
			
			$val = trim(COption::GetOptionString('imaginweb.sms', 'gate'.$siteId));
			if($val == '') {
				#COption::SetOptionString('imaginweb.sms', 'gate'.$siteId, $gateDef);
		...
		foreach($arSites as $siteId => $arVal) {
			$val = trim(COption::GetOptionString('imaginweb.sms', 'gate'.$siteId));
			if($val != '' && $val != 'OFF') {
				#COption::SetOptionString('imaginweb.sms', 'gate', $val);
				break;
			}
		}
	}
	...
	if($REQUEST_METHOD == "POST" && strlen($Update.$Apply.$RestoreDefaults) > 0 && $POST_RIGHT == "W" && check_bitrix_sessid()){
		...
		$old_processing_events_d7 = COption::GetOptionString('imaginweb.sms', 'processing_events_d7', 'Y');
		...
		$processing_events_d7 = COption::GetOptionString('imaginweb.sms', 'processing_events_d7', 'Y');
		
		if($old_processing_events_d7 != $processing_events_d7)
		{
			$eventManager = Bitrix\Main\EventManager::getInstance();
			
			if($processing_events_d7 == 'Y')
			{
				$eventManager->registerEventHandler('sale', 'OnSaleOrderPaid', 'imaginweb.sms', 'CImaginwebSms', 'OnSaleOrderPaidHandler');
				$eventManager->registerEventHandler('sale', 'OnShipmentAllowDelivery', 'imaginweb.sms', 'CImaginwebSms', 'OnShipmentAllowDeliveryHandler');
				$eventManager->registerEventHandler('sale', 'OnSaleOrderCanceled', 'imaginweb.sms', 'CImaginwebSms', 'OnSaleOrderCanceledHandler');
				$eventManager->registerEventHandler('sale', 'OnSaleStatusOrderChange', 'imaginweb.sms', 'CImaginwebSms', 'OnSaleStatusOrderChangeHandler');
			}
			else
			{
				$eventManager->unRegisterEventHandler('sale', 'OnSaleOrderPaid', 'imaginweb.sms', 'CImaginwebSms', 'OnSaleOrderPaidHandler');
				$eventManager->unRegisterEventHandler('sale', 'OnShipmentAllowDelivery', 'imaginweb.sms', 'CImaginwebSms', 'OnShipmentAllowDeliveryHandler');
				$eventManager->unRegisterEventHandler('sale', 'OnSaleOrderCanceled', 'imaginweb.sms', 'CImaginwebSms', 'OnSaleOrderCanceledHandler');
				$eventManager->unRegisterEventHandler('sale', 'OnSaleStatusOrderChange', 'imaginweb.sms', 'CImaginwebSms', 'OnSaleStatusOrderChangeHandler');
			}
		}
			...
			if(!window.jQuery) {
				...
				$(document).ready(function(){
					$('.balance').each(function(){
						...
						var obj = $(this);
						var tlogin = $(this).attr('tlogin');
						if (tlogin.length > 0) {
							var login = $('input[name="settings['+tlogin+']"]').val();
						} else {
							var login = $('input[name="settings[username_'+gateAttr+']"]').val();
						}
						
						//console.log(login+'  ' + gateAttr);
						if (login.length > 0) {
	...
	<? $tabControl->BeginNextTab() ?>
		...
		<tr>
			<td width="50%" class="field-name"><?=GetMessage("IMAGINWEB_SMS_UPRAVLENIE_SLUZOM_S_2")?><span style="color: red;">*</span></td>
			<td width="50%">
				<select name="settings[gate]">
					
					<option value="OFF" style="color:red;"><?=GetMessage("IMAGINWEB_SMS_RASSYLKA_OTKLUCENA")?></option>
					<option value="instam.ru" <?=((COption::GetOptionString('imaginweb.sms', 'gate') == 'instam.ru'))?' selected="selected"':''?>>instam.ru</option>
					<option value="devinotele.com" <?=((COption::GetOptionString('imaginweb.sms', 'gate') == 'devinotele.com'))?' selected="selected"':''?>>devinotele.com</option>
					<option value="p1sms.ru" <?=((COption::GetOptionString('imaginweb.sms', 'gate') == 'p1sms.ru'))?' selected="selected"':''?>>p1sms.ru</option>
					<option value="qtelecom.ru" <?=((COption::GetOptionString('imaginweb.sms', 'gate') == 'qtelecom.ru'))?' selected="selected"':''?>>qtelecom.ru</option>
					<option value="smsdirect.ru" <?=((COption::GetOptionString('imaginweb.sms', 'gate') == 'smsdirect.ru'))?' selected="selected"':''?>>smsdirect.ru</option>
					<option value="redsms.ru" <?=((COption::GetOptionString('imaginweb.sms', 'gate') == 'redsms.ru'))?' selected="selected"':''?>>redsms.ru</option>
					<option value="redsms.ru2.0" <?=((COption::GetOptionString('imaginweb.sms', 'gate') == 'redsms.ru2.0'))?' selected="selected"':''?>>redsms.ru2.0</option>
					...
					<option value="nssms.ru2" <?=((COption::GetOptionString('imaginweb.sms', 'gate') == 'nssms.ru2'))?' selected="selected"':''?>>nssms.ru2</option>
					<option value="mobilmoney.ru" <?=(COption::GetOptionString('imaginweb.sms', 'gate') == 'mobilmoney.ru')?' selected="selected"':''?>>mobilmoney.ru</option>
		...
		<tr>
			<td width="50%" class="field-name"><?=GetMessage("IMAGINWEB_SMS_OBRABATYVATQ_SOOBSEN")?> php <?=GetMessage("IMAGINWEB_SMS_KOD")?><span style="color: red;">*</span></td>
			<td width="50%">
				<select name="settings[make_php]">
					<option value="N" style="color:red;"><?=GetMessage("IMAGINWEB_SMS_NET")?></option>
					<option value="Y" <?=((COption::GetOptionString('imaginweb.sms', 'make_php') == 'Y'))?' selected="selected"':''?>><?=GetMessage("IMAGINWEB_SMS_DA_VSE_SOOBSENIA_BU")?><?=GetMessage("IMAGINWEB_SMS_KOD_PERED_OTPRAVKOY")?></option>
				</select>
			</td>
		</tr>
		<? if(CModule::IncludeModule("sale")) :?>
		...
		<? if(CModule::IncludeModule("sale")) :?>
			<tr>
				<td class="adm-detail-content-cell-l">
					<label for="PROCESSING_EVENTS_D7"><?=GetMessage("IMAGINWEB_PROCESSING_EVENTS_D7")?></label>
				</td>
				<td class="adm-detail-content-cell-r">
					<input type="hidden" name="settings[processing_events_d7]" value="N" />
					<?
					$val = COption::GetOptionString('imaginweb.sms', 'processing_events_d7', 'Y');
					?>
					<input type="checkbox" name="settings[processing_events_d7]" id="PROCESSING_EVENTS_D7" value="Y"<?if($val == 'Y')echo' checked';?>>
				</td>
			</tr>
			...
			<tr id="add-field" style="display: none;" class="heading">
				...
				<td width="50%" >
					<div class="tal-right">
						...
						<?if($arOrderFields)
						{
							$arOrderTemplate = array();
							foreach($arOrderFields as $k => $v)
								$arOrderTemplate[] = '#'.$v.'#';
							echo implode(', ', $arOrderTemplate).'<br/><br/>';
						}?>
		...
		<? foreach($arSites as $id => $value):?>
			...
			<tr id="site<?=$id?>" <?=(COption::GetOptionString('imaginweb.sms', 'site')==$id)?'':'style="display:none"'?> class="sites">
				...
				<table class="edit-table">
					<tr>
						<td width="50%" class="field-name"><?=GetMessage("IMAGINWEB_SMS_UPRAVLENIE_SLUZOM_S")?><span style="color: red;">*</span></td>
						<td width="50%">
							<select name="settings[gate<?=$id?>]">
								
								<option value="OFF" style="color:red;"><?=GetMessage("IMAGINWEB_SMS_RASSYLKA_OTKLUCENA")?></option>
								<option value="instam.ru" <?=((COption::GetOptionString('imaginweb.sms', 'gate'.$id) == 'instam.ru'))?' selected="selected"':''?>>instam.ru</option>
								<option value="devinotele.com" <?=((COption::GetOptionString('imaginweb.sms', 'gate'.$id) == 'devinotele.com'))?' selected="selected"':''?>>devinotele.com</option>
								<option value="p1sms.ru" <?=((COption::GetOptionString('imaginweb.sms', 'gate'.$id) == 'p1sms.ru'))?' selected="selected"':''?>>p1sms.ru</option>
								<option value="qtelecom.ru" <?=((COption::GetOptionString('imaginweb.sms', 'gate'.$id) == 'qtelecom.ru'))?' selected="selected"':''?>>qtelecom.ru</option>
								<option value="smsdirect.ru" <?=((COption::GetOptionString('imaginweb.sms', 'gate'.$id) == 'smsdirect.ru'))?' selected="selected"':''?>>smsdirect.ru</option>
								<option value="redsms.ru" <?=((COption::GetOptionString('imaginweb.sms', 'gate'.$id) == 'redsms.ru'))?' selected="selected"':''?>>redsms.ru</option>
								<option value="redsms.ru2.0" <?=((COption::GetOptionString('imaginweb.sms', 'gate'.$id) == 'redsms.ru2.0'))?' selected="selected"':''?>>redsms.ru2.0</option>
								<option value="am4u.ru" <?=((COption::GetOptionString('imaginweb.sms', 'gate'.$id) == 'am4u.ru'))?' selected="selected"':''?>>am4u.ru</option>
								<option value="sms-sending.ru" <?=((COption::GetOptionString('imaginweb.sms', 'gate'.$id) == 'sms-sending.ru'))?' selected="selected"':''?>>sms-sending.ru</option>
					
								
								<option value="alfa-sms.ru" <?=((COption::GetOptionString('imaginweb.sms', 'gate'.$id) == 'alfa-sms.ru'))?' selected="selected"':''?>>alfa-sms.ru</option>
								<option value="mainsms.ru" <?=((COption::GetOptionString('imaginweb.sms', 'gate'.$id) == 'mainsms.ru'))?' selected="selected"':''?>>mainSMS.ru</option>
								<!--<option value="kompeito.ru" <?=((COption::GetOptionString('imaginweb.sms', 'gate'.$id) == 'kompeito.ru'))?' selected="selected"':''?>>kompeito.ru</option>-->
								<option value="infosmska.ru" <?=((COption::GetOptionString('imaginweb.sms', 'gate'.$id) == 'infosmska.ru'))?' selected="selected"':''?>>infoSMSka.ru</option>
								<option value="bytehand.com" <?=((COption::GetOptionString('imaginweb.sms', 'gate'.$id) == 'bytehand.com'))?' selected="selected"':''?>>bytehand.com</option>
								<option value="imobis" <?=((COption::GetOptionString('imaginweb.sms', 'gate'.$id) == 'imobis'))?' selected="selected"':''?>>Imobis.ru</option>
								<option value="axtele.com" <?=((COption::GetOptionString('imaginweb.sms', 'gate'.$id) == 'axtele.com'))?' selected="selected"':''?>>axtele.com</option>
								<option value="nssms.ru" <?=((COption::GetOptionString('imaginweb.sms', 'gate'.$id) == 'nssms.ru'))?' selected="selected"':''?>>nssms.ru</option>
								<option value="nssms.ru2" <?=((COption::GetOptionString('imaginweb.sms', 'gate'.$id) == 'nssms.ru2'))?' selected="selected"':''?>>nssms.ru2</option>
								<option value="mobilmoney.ru" <?=(COption::GetOptionString('imaginweb.sms', 'gate'.$id) == 'mobilmoney.ru')?' selected="selected"':''?>>mobilmoney.ru</option>
								<option value="turbosms.ua" <?=((COption::GetOptionString('imaginweb.sms', 'gate'.$id) == 'turbosms.ua'))?' selected="selected"':''?>>turbosms.ua</option>
								<option value="epochtasms" <?=((COption::GetOptionString('imaginweb.sms', 'gate'.$id) == 'epochtasms'))?' selected="selected"':''?>>ePochtaSMS</option>
								<option value="giper.mobi" <?=((COption::GetOptionString('imaginweb.sms', 'gate'.$id) == 'giper.mobi'))?' selected="selected"':''?>>giper.mobi</option>
							</select>
						</td>
					</tr>
					<tr>
		...
		<? if(CIWebSMS::checkPhpVer53()):?>
			...
			<? endif?>
				<tr>
					...
					<td width="50%">
						<select name="settings[user_password_field2]" class="user_password_field">
							<option value="OFF" style="color:red;"><?=GetMessage("IMAGINWEB_SMS_NE_SOHRANATQ")?></option>
							<? foreach($arUserFields as $field):?>
								<option value="<?=$field['FIELD_NAME']?>" <?=((COption::GetOptionString('imaginweb.sms', 'user_password_field2') == $field['FIELD_NAME']))?' selected="selected"':''?>><?=$field['FIELD_NAME']?></option>
							<? endforeach;?>
						</select>
					</td>
				</tr>
...
<!--instam.ru-->
		<tr class="heading gateLinkHead">
			<td colspan="2"><a class="toggle-link gateControll-new" ref="instam.ru"  href="javascript:iwebtoggle('instam.ru')">instam.ru</a></td>
		</tr>
		
		<tr class="sites gateShow instam.ru" id="instam.ru" style="display: none;">
			<td colspan="2">
				<table class="edit-table">
					<tr class="heading">
					...
					<tr>
						<td colspan="2">
							<div class="tab-text">
								<p class='iweb-medium'>
									<?=GetMessage("IMAGINWEB_SMS_PROMO_INSTAM")?>
								</p>
							</div>
						</td>
					</tr>
					<tr>
						...
						<td width="50%">
							<input
								type="text"
								size="30"
								value="<?=(COption::GetOptionString('imaginweb.sms', 'originator_instam.ru'))?COption::GetOptionString('imaginweb.sms', 'originator_instam.ru'):'SMSINFO' ?>"
								name="settings[originator_instam.ru]" />
						</td>
					</tr>
					<tr class="heading">
					...
					<tr>
						...
						<td width="50%">
						    <input
							    type="text"
							    size="30"
							    value="<?=COption::GetOptionString('imaginweb.sms', 'username_instam.ru') ?>"
							    name="settings[username_instam.ru]" />
						</td>
					</tr>
					<tr>
						...
						<td width="50%">
						    <input
							    type="password"
							    size="30"
							    value="<?=COption::GetOptionString('imaginweb.sms', 'password_instam.ru')?>"
							    name="settings[password_instam.ru]" />
						</td>
					</tr>
					<tr>
						<td width="50%"><strong><?=GetMessage("IMAGINWEB_SMS_BALANS_RUB")?></strong></td>
						<td width="50%" tlogin=""  class="balance" gate="instam.ru"></td>
					</tr>
				</table>
			</td>
		</tr>
<!--devinotele.com-->
		<tr class="heading gateLinkHead">
			<td colspan="2"><a class="toggle-link gateControll-new" ref="devinotele.com"  href="javascript:iwebtoggle('devinotele.com')">devinotele.com</a></td>
		</tr>
		
		<tr class="sites gateShow devinotele.com" id="devinotele.com" style="display: none;">
			<td colspan="2">
				<table class="edit-table">
					<tr class="heading">
						<td colspan="2"><?=GetMessage("IMAGINWEB_SMS_NASTROYKI_PODKLUCENI")?></td>
					...
					<tr>
						<td colspan="2">
							<div class="tab-text">
								<p class='iweb-medium'>
									<?=GetMessage("IMAGINWEB_SMS_PROMO_DEVINOTELECOM")?>
								</p>
							</div>
						</td>
					</tr>
					<tr>
						...
						<td width="50%">
							<input
								type="text"
								size="30"
								value="<?=(COption::GetOptionString('imaginweb.sms', 'originator_devinotele.com'))?COption::GetOptionString('imaginweb.sms', 'originator_devinotele.com'):'DTSMS' ?>"
								name="settings[originator_devinotele.com]" />
						</td>
					</tr>
					<tr class="heading">
						<td colspan="2"><?=GetMessage("IMAGINWEB_SMS_NASTROYKI_DOSTUPA")?></td>
					...
					<tr>
						...
						<td width="50%">
						    <input
							    type="text"
							    size="30"
							    value="<?=COption::GetOptionString('imaginweb.sms', 'username_devinotele.com') ?>"
							    name="settings[username_devinotele.com]" />
						</td>
					</tr>
					<tr>
						<td width="50%"><?=GetMessage("IMAGINWEB_SMS_PAROLQ")?></td>
						<td width="50%">
						    <input
							    type="password"
							    size="30"
							    value="<?=COption::GetOptionString('imaginweb.sms', 'password_devinotele.com')?>"
							    name="settings[password_devinotele.com]" />
						</td>
					</tr>
					<tr>
						<td width="50%"><strong><?=GetMessage("IMAGINWEB_SMS_BALANS_RUB")?></strong></td>
						<td width="50%" tlogin=""  class="balance" gate="devinotele.com"></td>
					</tr>
				</table>
			</td>
		</tr>
<!--p1sms.ru-->
		<tr class="heading gateLinkHead">
			<td colspan="2"><a class="toggle-link gateControll-new" ref="p1sms.ru"  href="javascript:iwebtoggle('p1sms.ru')">p1sms.ru</a></td>
		</tr>
		
		<tr class="sites gateShow p1sms.ru" id="p1sms.ru" style="display: none;">
			<td colspan="2">
				<table class="edit-table">
					<tr class="heading">
						<td colspan="2"><?=GetMessage("IMAGINWEB_SMS_NASTROYKI_PODKLUCENI")?></td>
					...
					<tr>
						<td colspan="2">
							<div class="tab-text">
								<p class='iweb-medium'>
									<div style="transition: color 1s ease 0s; color: rgb(124, 74, 255);" class="text_1" id="send_sms_a"><b><?=GetMessage("IMAGINWEB_SMS_RABOTAEM_PO_VSEM_STR1")?></b></div>
									<script>
									
									document.all("send_sms_a").className="text_1";
									var heading = document.all("send_sms_a");
								...
								<p class='iweb-medium'>
									<a href="http://p1sms.ru/" target="_blank">P1sms.ru</a> <?=GetMessage("IMAGINWEB_SMS_ETO_SERVIS_SMS_RAS")?> 1C-BITRIX <?=GetMessage("IMAGINWEB_SMS_VY_SMOJETE_INTEGRIRO")?>.
								</p>
							</div>
						</td>
					</tr>
					<tr>
						...
						<td width="50%">
							<input
								type="text"
								size="30"
								value="<?=(COption::GetOptionString('imaginweb.sms', 'originator_p1sms.ru'))?COption::GetOptionString('imaginweb.sms', 'originator_p1sms.ru'):'NOVOSTI' ?>"
								name="settings[originator_p1sms.ru]" />
						</td>
					</tr>
					<tr class="heading">
						<td colspan="2"><?=GetMessage("IMAGINWEB_SMS_NASTROYKI_DOSTUPA")?></td>
					...
					<tr>
						...
						<td width="50%">
						    <input
							    type="text"
							    size="30"
							    value="<?=COption::GetOptionString('imaginweb.sms', 'username_p1sms.ru') ?>"
							    name="settings[username_p1sms.ru]" />
						</td>
					</tr>
					<tr>
						<td width="50%"><?=GetMessage("IMAGINWEB_SMS_PAROLQ")?></td>
						<td width="50%">
						    <input
							    type="password"
							    size="30"
							    value="<?=COption::GetOptionString('imaginweb.sms', 'password_p1sms.ru')?>"
							    name="settings[password_p1sms.ru]" />
						</td>
					</tr>
					<tr>
						<td width="50%"><strong><?=GetMessage("IMAGINWEB_SMS_BALANS_RUB")?></strong></td>
						<td width="50%" tlogin=""  class="balance" gate="p1sms.ru"></td>
					</tr>
				</table>
			</td>
		</tr>
		...
		<tr class="sites gateShow redsms.ru" id="redsms.ru" style="display: none;">
			<td colspan="2">
				<table class="edit-table">
					...
					<tr>
						<td width="50%"><strong><?=GetMessage("IMAGINWEB_SMS_BALANS_RUB")?></strong></td>
						<td width="50%" tlogin=""  class="balance" gate="redsms.ru"></td>
					</tr>
				</table>
			</td>
		</tr>
		...
		<!--redsms.ru2.0-->
		<tr class="heading gateLinkHead">
			<td colspan="2"><a class="toggle-link gateControll-new" ref="redsms.ru2.0"  href="javascript:iwebtoggle('redsms.ru2.0')">redsms.ru2.0</a></td>
		</tr>
		
		<tr class="sites gateShow redsms.ru2.0" id="redsms.ru2.0" style="display: none;">
			<td colspan="2">
				<table class="edit-table">
					<tr class="heading">
						<td colspan="2"><?=GetMessage("IMAGINWEB_SMS_NASTROYKI_PODKLUCENI")?></td>
					...
					<tr>
						...
						<td width="50%">
							<input
								type="text"
								size="30"
								value="<?=(COption::GetOptionString('imaginweb.sms', 'originator_redsms.ru2.0'))?COption::GetOptionString('imaginweb.sms', 'originator_redsms.ru2.0'):'smstest' ?>"
								name="settings[originator_redsms.ru2.0]" />
						</td>
					</tr>
					<tr class="heading">
						<td colspan="2"><?=GetMessage("IMAGINWEB_SMS_NASTROYKI_DOSTUPA")?></td>
					...
					<tr>
						...
						<td width="50%">
						    <input
							    type="text"
							    size="30"
							    value="<?=COption::GetOptionString('imaginweb.sms', 'username_redsms.ru2.0') ?>"
							    name="settings[username_redsms.ru2.0]" />
						</td>
					</tr>
					<tr>
						<td width="50%"><?=GetMessage("IMAGINWEB_SMS_API_KEY")?></td>
						<td width="50%">
						    <input
							    type="password"
							    size="30"
							    value="<?=COption::GetOptionString('imaginweb.sms', 'password_redsms.ru2.0')?>"
							    name="settings[password_redsms.ru2.0]" />
						</td>
					</tr>
					<tr>
						<td width="50%"><strong><?=GetMessage("IMAGINWEB_SMS_BALANS_RUB")?></strong></td>
						<td width="50%" tlogin=""  class="balance" gate="redsms.ru2.0"></td>
					</tr>
				</table>
			</td>
		</tr>
		...
		<!--qtelecom.ru-->
		<tr class="heading gateLinkHead">
			<td colspan="2"><a class="toggle-link gateControll-new" ref="qtelecom.ru"  href="javascript:iwebtoggle('qtelecom.ru')">qtelecom.ru</a></td>
		</tr>
		
		<tr class="sites gateShow qtelecom.ru" id="qtelecom.ru" style="display: none;">
			<td colspan="2">
				<table class="edit-table">
					<tr class="heading">
						<td colspan="2"><?=GetMessage("IMAGINWEB_SMS_NASTROYKI_PODKLUCENI")?></td>
					...
					<tr>
						<td colspan="2">
							<div class="tab-text">
								<p class='iweb-medium'>
									<?=GetMessage("IMAGINWEB_SMS_PROMO_QTELECOM")?>
								</p>
							</div>
						</td>
					</tr>
					<tr>
						...
						<td width="50%">
							<input
								type="text"
								size="30"
								value="<?=(COption::GetOptionString('imaginweb.sms', 'originator_qtelecom.ru'))?COption::GetOptionString('imaginweb.sms', 'originator_qtelecom.ru'):'test-sms' ?>"
								name="settings[originator_qtelecom.ru]" />
						</td>
					</tr>
					<tr class="heading">
						<td colspan="2"><?=GetMessage("IMAGINWEB_SMS_NASTROYKI_DOSTUPA")?></td>
					...
					<tr>
						...
						<td width="50%">
						    <input
							    type="text"
							    size="30"
							    value="<?=COption::GetOptionString('imaginweb.sms', 'username_qtelecom.ru') ?>"
							    name="settings[username_qtelecom.ru]" />
						</td>
					</tr>
					<tr>
						<td width="50%"><?=GetMessage("IMAGINWEB_SMS_PAROLQ")?></td>
						<td width="50%">
						    <input
							    type="password"
							    size="30"
							    value="<?=COption::GetOptionString('imaginweb.sms', 'password_qtelecom.ru')?>"
							    name="settings[password_qtelecom.ru]" />
						</td>
					</tr>
					<tr>
						<td width="50%"><strong><?=GetMessage("IMAGINWEB_SMS_BALANS_RUB")?></strong></td>
						<td width="50%" tlogin=""  class="balance" gate="qtelecom.ru"></td>
					</tr>
				</table>
			</td>
		</tr>
		...
		<!--smsdirect.ru-->
		<tr class="heading gateLinkHead">
			<td colspan="2"><a class="toggle-link gateControll-new" ref="smsdirect.ru"  href="javascript:iwebtoggle('smsdirect.ru')">smsdirect.ru</a></td>
		</tr>
		
		<tr class="sites gateShow smsdirect.ru" id="smsdirect.ru" style="display: none;">
			<td colspan="2">
				<table class="edit-table">
					<tr class="heading">
						<td colspan="2"><?=GetMessage("IMAGINWEB_SMS_NASTROYKI_PODKLUCENI")?></td>
...
- <?=GetMessage("IMAGINWEB_SMS_MGNOVENNOE_OPOVESENI")?><br/><br/>

- <?=GetMessage("IMAGINWEB_SMS_OTPRAVKA_SMS_PO_VSEM")?><br/><br/>

- <?=GetMessage("IMAGINWEB_SMS_TOLQKO_PRAMYE_PODKLU")?><br/><br/>

<?=GetMessage("IMAGINWEB_SMS_SERVIS")?> SMSDirect <?=GetMessage("IMAGINWEB_SMS_POZVOLAET_OTSLEJIVAT")?> on-line <?=GetMessage("IMAGINWEB_SMS_REJIME_OBESPECIVAET")?>: e-mail-<?=GetMessage("IMAGINWEB_SMS_RASSYLKA_GOLOSOVYE")?><br/><br/>

<?=GetMessage("IMAGINWEB_SMS_ZA_PODROBNOY_INFORMA")?>: info@smsdirect.ru , 8-800-555-35-88<br/><br/>
<a target="_blank" href="http://smsdirect.ru/">smsdirect.ru</a>
								</p>
							</div>
						</td>
					</tr>
					<tr>
						...
						<td width="50%">
							<input
								type="text"
								size="30"
								value="<?=(COption::GetOptionString('imaginweb.sms', 'originator_smsdirect.ru'))?COption::GetOptionString('imaginweb.sms', 'originator_smsdirect.ru'):'SMSDirect' ?>"
								name="settings[originator_smsdirect.ru]" />
						</td>
					</tr>
					<tr class="heading">
						<td colspan="2"><?=GetMessage("IMAGINWEB_SMS_NASTROYKI_DOSTUPA")?></td>
					...
					<tr>
						...
						<td width="50%">
						    <input
							    type="text"
							    size="30"
							    value="<?=COption::GetOptionString('imaginweb.sms', 'username_smsdirect.ru') ?>"
							    name="settings[username_smsdirect.ru]" />
						</td>
					</tr>
					<tr>
						<td width="50%"><?=GetMessage("IMAGINWEB_SMS_PAROLQ")?></td>
						<td width="50%">
						    <input
							    type="password"
							    size="30"
							    value="<?=COption::GetOptionString('imaginweb.sms', 'password_smsdirect.ru')?>"
							    name="settings[password_smsdirect.ru]" />
						</td>
					</tr>
					<tr>
						<td width="50%"><strong><?=GetMessage("IMAGINWEB_SMS_BALANS_RUB")?></strong></td>
						<td width="50%" tlogin=""  class="balance" gate="smsdirect.ru"></td>
					</tr>
				</table>
			</td>
		</tr>
...
<?=GetMessage("IMAGINWEB_SMS_SMS_RASSYLKI")?> &laquo;<?=GetMessage("IMAGINWEB_SMS_ALEF_MARKETING_SERVI")?>&raquo;
  ...
  <?=GetMessage("IMAGINWEB_SMS_LICNOGO_KABINETA_TR")?> XML <?=GetMessage("IMAGINWEB_SMS_PROTOKOL_I_ISPOLQZ")?>.
					...
					<tr>
						<td width="50%"><strong><?=GetMessage("IMAGINWEB_SMS_BALANS_RUB")?></strong></td>
						<td width="50%" tlogin="username11"  class="balance" gate="am4u.ru"></td>
					</tr>
				</table>
			</td>
		</tr>
...
<?=GetMessage("IMAGINWEB_SMS_UDOBNYY_I_KACESTVENN")?> E-mail, <?=GetMessage("IMAGINWEB_SMS_UVEDOMLENIA_OB")?> Email, <?=GetMessage("IMAGINWEB_SMS_ONLAYN_KONSULQTANT")?>". 50 <?=GetMessage("IMAGINWEB_SMS_SMS_DLA_TESTA")?><br/><br/><?=GetMessage("IMAGINWEB_SMS_DLA_PODKLUCENIA_TARI")?>.
					...
					<tr>
						<td width="50%"><strong><?=GetMessage("IMAGINWEB_SMS_BALANS_RUB")?></strong></td>
						<td width="50%" tlogin=""  class="balance" gate="sms-sending.ru"></td>
					</tr>
				</table>
			</td>
		</tr>
		...
		<tr class="sites gateShow alfa-sms.ru" id="alfa-sms.ru" style="display: none;">
			<td colspan="2">
				<table class="edit-table">
					...
					<tr>
						<td width="50%"><strong><?=GetMessage("IMAGINWEB_SMS_BALANS_RUB")?></strong></td>
						<td width="50%" tlogin=""  class="balance" gate="alfa-sms.ru"></td>
					</tr>
				</table>
			</td>
		</tr>
		...
		<tr class="sites gateShow mainSMS" id="mainSMS" style="display: none;">
			<td colspan="2">
				<table class="edit-table">
					...
					<tr>
						<td width="50%"><strong><?=GetMessage("IMAGINWEB_SMS_BALANS_RUB")?></strong></td>
						<td width="50%" tlogin="username10"  class="balance" gate="mainsms.ru"></td>
					</tr>
				</table>
			</td>
		</tr>
...
<?=GetMessage("IMAGINWEB_SMS_VVEDITE_V_POLE_NIJE")?>.
					...
					<tr>
						...
						<td width="50%" tlogin="username9"  class="balance" gate="kompeito.ru"></td>
					</tr>
				</table>
			</td>
		</tr>
		...
		<tr class="sites gateShow infosmska" id="infosmska" style="display: none;">
			...
			<?=GetMessage("IMAGINWEB_SMS_DLA_ETOGO_VOYDITE_V")?></p></div>
					...
					<tr>
						...
						<td width="50%" tlogin="username4"  class="balance" gate="infosmska.ru"></td>
					</tr>
					<tr>
	...
	<?# $tabControl->BeginNextTab() ?>
		...
		<tr class="sites gateShow bytehand" id="bytehand" style="display: none;">
			...
			<?=GetMessage("IMAGINWEB_SMS_OPERATIVNAA")?> online <?=GetMessage("IMAGINWEB_SMS_PODDERJKA")?></p></div></td>
					...
					<tr>
						<td width="50%"><strong><?=GetMessage("IMAGINWEB_SMS_BALANS_RUBLEY")?></strong></td>
						<td width="50%" tlogin="username8"  class="balance" gate="bytehand.com"></td>
					</tr>
				</table>
			</td>
		</tr>
		...
		<tr class="sites gateShow imobis" id="imobis" style="display: none;">
			<td colspan="2">
				<table class="edit-table">
					...
					<tr>
						<td width="50%"><strong><?=GetMessage("IMAGINWEB_SMS_BALANS_RUBLEY")?></strong></td>
						<td width="50%" tlogin="username6"  class="balance" gate="imobis"></td>
					</tr>
				</table>
			</td>
		</tr>
	...
	<?# $tabControl->BeginNextTab() ?>
		...
		<tr class="sites gateShow axtele" id="axtele" style="display: none;">
			<td colspan="2">
				<table class="edit-table">
					...
					<tr>
						<td width="50%"><strong><?=GetMessage("IMAGINWEB_SMS_BALANS_RUBLEY")?></strong></td>
						<td width="50%" tlogin="username5"  class="balance" gate="axtele.com"></td>
					</tr>
				</table>
			</td>
		</tr>
		...
		<tr class="sites gateShow nssms.ru" id="nssms.ru" style="display: none;">
			<td colspan="2">
				<table class="edit-table">
					...
					<tr>
						<td width="50%"><strong><?=GetMessage("IMAGINWEB_SMS_BALANS_RUB")?></strong></td>
						<td width="50%" tlogin=""  class="balance" gate="nssms.ru"></td>
					</tr>
				</table>
			</td>
		</tr>
		...
		<!--nssms.ru2-->
		<tr class="heading gateLinkHead">
			<td colspan="2"><a class="toggle-link gateControll-new" ref="nssms.ru2"  href="javascript:iwebtoggle('nssms.ru2')">nssms.ru2</a></td>
		</tr>
		
		<tr class="sites gateShow nssms.ru2" id="nssms.ru2" style="display: none;">
			<td colspan="2">
				<table class="edit-table">
					<tr class="heading">
						<td colspan="2"><?=GetMessage("IMAGINWEB_SMS_NASTROYKI_PODKLUCENI")?></td>
					...
					<tr>
						...
						<td width="50%">
							<input
								type="text"
								size="30"
								value="<?=(COption::GetOptionString('imaginweb.sms', 'originator_nssms.ru2'))?COption::GetOptionString('imaginweb.sms', 'originator_nssms.ru2'):'' ?>"
								name="settings[originator_nssms.ru2]" />
						</td>
					</tr>
					<tr class="heading">
						<td colspan="2"><?=GetMessage("IMAGINWEB_SMS_NASTROYKI_DOSTUPA")?></td>
					...
					<tr>
						...
						<td width="50%">
						    <input
							    type="text"
							    size="30"
							    value="<?=COption::GetOptionString('imaginweb.sms', 'username_nssms.ru2') ?>"
							    name="settings[username_nssms.ru2]" />
						</td>
					</tr>
					<tr>
						<td width="50%"><?=GetMessage("IMAGINWEB_SMS_PAROLQ")?></td>
						<td width="50%">
						    <input
							    type="password"
							    size="30"
							    value="<?=COption::GetOptionString('imaginweb.sms', 'password_nssms.ru2')?>"
							    name="settings[password_nssms.ru2]" />
						</td>
					</tr>
					<tr>
						<td width="50%"><strong><?=GetMessage("IMAGINWEB_SMS_BALANS_RUB")?></strong></td>
						<td width="50%" tlogin=""  class="balance" gate="nssms.ru2"></td>
					</tr>
				</table>
			</td>
		</tr>
		...
		<tr class="sites gateShow mobilemoney" id="mobilemoney" style="display: none;">
			<td colspan="2">
				<table class="edit-table">
					...
					<tr>
						<td width="50%"></td>
						<td width="50%" tlogin="username"  class="balance" gate="mobilmoney.ru"></td>
					</tr>
				</table>
			</td>
		</tr>
			...
			<td colspan="2">
				<table class="edit-table">
					...
					<tr>
						...
						<td width="50%" tlogin="username2"  class="balance" gate="turbosms.ua"></td>
					</tr>
				</table>
			</td>
		</tr>
		...
		<tr class="sites gateShow epochtasms" id="epochtasms" style="display: none;">
			<td colspan="2">
				<table class="edit-table">
					...
					<tr>
						<td width="50%"><strong><?=GetMessage("IMAGINWEB_SMS_BALANS_KREDITOV")?></strong></td>
						<td width="50%" tlogin="username3"  class="balance" gate="epochtasms"></td>
					</tr>
				</table>
			</td>
		</tr>
		...
		<tr class="sites gateShow giper" id="giper" style="display: none;">
			<td colspan="2">
				<table class="edit-table">
					...
					<tr>
						<td width="50%"><strong><?=GetMessage("IMAGINWEB_SMS_BALANS_RUBLEY")?></strong></td>
						<td width="50%" tlogin="username7"  class="balance" gate="giper.mobi"></td>
					</tr>
				</table>
			</td>
		</tr>
