...
class smsSendingTransport{
	...
	function send($params = array(), $phones = array()){
		...
		$someXML = "";
		if (isset($params["text"])) $params["text"] = htmlspecialchars($params["text"],null,IWEB_SMS_SENDING_HTTPS_CHARSET);
		foreach ($phones as $phone){
			if (is_array($phone)){
				if (isset($phone["number"])){
					$someXML .= "<to number='".$phone['number']."'>";
					if (isset($phone["text"])){
						$someXML .= htmlspecialchars($phone["text"],null,IWEB_SMS_SENDING_HTTPS_CHARSET);
					}
					$someXML .= "</to>";
				}
			}
	...
	function getURL($action){
		if (USE_HTTPS == 1)
            $address = IWEB_SMS_SENDING_HTTPS_ADDRESS."API/XML/".$action.".php";
        else
            $address = IWEB_SMS_SENDING_HTTP_ADDRESS."API/XML/".$action.".php";
	...
	function request($action,$params = array(),$someXML = ""){
		...
		if (IWEB_SMS_SENDING_HTTPS_METHOD == "curl"){
			$res = $this->request_curl($action,$xml);
		}
		elseif (IWEB_SMS_SENDING_HTTPS_METHOD == "file_get_contents"){
			$opts = array('http' =>
			    array(
			        'method'  => 'POST',
		...
		$this->error(GetMessage("IMAGINWEB_SMS_V_NASTROYKAH_UKAZAN").IWEB_SMS_SENDING_HTTPS_METHOD."'");
	}
	...
	function getConvertedString($value, $from = false){
		if (IWEB_SMS_SENDING_HTTPS_CHARSET_AUTO_DETECT){
			if (!$this->charset){
				$this->charset = $this->detectCharset($value);
			}	
		}
		else{
			$this->charset = IWEB_SMS_SENDING_HTTPS_CHARSET;
		}
	...
	function __construct($login,$password){
		require_once(dirname(__FILE__)."/sms-sending_config.php");
		$this->LOGIN = $login;
