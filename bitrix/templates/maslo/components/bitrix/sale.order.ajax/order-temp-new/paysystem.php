<?if(!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED!==true)die();?>
 ...
 <? endif?>
<div class="section paysistem-list">
	...
	<div class="bx_section">
		...
		if ($arResult["PAY_FROM_ACCOUNT"] == "Y")
		{
			...
			<div class="bx_block w100 vertical">
				<div class="checkbox">
					<input type="hidden" name="PAY_CURRENT_ACCOUNT" value="N"><i></i>
		...
		{
			if ((in_array(19, $USER->GetUserGroupArray()) == "1")) {
						...
						<div class="bx_block w100 vertical">
							<div class="checkbox">
								...
								<input type="radio"
									...
									/><i onclick="BX('ID_PAY_SYSTEM_ID_<?=$arPaySystem["ID"]?>').checked=true;changePaySystem();"></i>
						...
						<div class="bx_block w100 vertical">
							<div class="checkbox">
								<input type="radio"
									...
									value="<?=$arPaySystem["ID"]?>"
									<?if ($arPaySystem["CHECKED"]=="Y" && !($arParams["ONLY_FULL_PAY_FROM_ACCOUNT"] == "Y" && $arResult["USER_VALS"]["PAY_CURRENT_ACCOUNT"]=="Y")) echo " checked=\"checked\"";?>
									onclick="changePaySystem();" /><i onclick="BX('ID_PAY_SYSTEM_ID_<?=$arPaySystem["ID"]?>').checked=true;changePaySystem();"></i>
								<label for="ID_PAY_SYSTEM_ID_<?=$arPaySystem["ID"]?>" onclick="BX('ID_PAY_SYSTEM_ID_<?=$arPaySystem["ID"]?>').checked=true;changePaySystem();">
									<?
									if (count($arPaySystem["PSA_LOGOTIP"]) > 0):
										$imgUrl = $arPaySystem["PSA_LOGOTIP"]["SRC"];
			...
			} elseif((in_array(19, $USER->GetUserGroupArray()) != "1") && ($arPaySystem["ID"] != "13") && ($arPaySystem["ID"] != "15") && ($arPaySystem["ID"] != "14")) {
				if (strlen(trim(str_replace("<br />", "", $arPaySystem["DESCRIPTION"]))) > 0 || intval($arPaySystem["PRICE"]) > 0)
				{
					if (count($arResult["PAY_SYSTEM"]) == 1)
					{
