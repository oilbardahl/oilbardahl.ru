		...
		do
		{
			...
			if ('' == $strErrorR && $bIBlockIsCatalog)
			{
				...
				if (empty($row))
				{
					...
					{
						$arLoadOfferArray['QUANTITY'] = 0;
						$arLoadOfferArray['QUANTITY_RESERVED'] = 0;
						$arLoadOfferArray['QUANTITY_TRACE'] = Catalog\ProductTable::STATUS_YES;
						$arLoadOfferArray['CAN_BUY_ZERO'] = Catalog\ProductTable::STATUS_NO;
					}
					else
					{
						if (isset($arLoadOfferArray['QUANTITY']) && trim($arLoadOfferArray['QUANTITY']) === '')
							unset($arLoadOfferArray['QUANTITY']);
						if (isset($arLoadOfferArray['QUANTITY_RESERVED']) && trim($arLoadOfferArray['QUANTITY_RESERVED']) === '')
							unset($arLoadOfferArray['QUANTITY_RESERVED']);
					}
