<?
$MESS["FORUM_CM_ERR_EMPTY_TEXT"] = "Не указан текст комментария.";
$MESS["FORUM_CM_ERR_EMPTY_AUTHORS_NAME"] = "Не указано имя автора.";
$MESS["FORUM_CM_ERR_COMMENT_IS_LOST1"] = "Комментарий для редактирования не найден.";
$MESS["FORUM_CM_ERR_COMMENT_IS_LOST2"] = "Комментарий для удаления не найден.";
$MESS["FORUM_CM_ERR_COMMENT_IS_LOST3"] = "Комментарий для модерирования не найден.";
$MESS["FORUM_CM_ERR_COMMENT_IS_LOST4"] = "Комментарий для проверки прав не найден.";
$MESS["ACCESS_DENIED"] = "Доступ запрещён.";
$MESS["FORUM_CM_ERR_DELETE"] = "Ошибка удаления комментария.";
$MESS["FORUM_CM_ERR_MODERATE"] = "Ошибка модерирования комментария.";
?>